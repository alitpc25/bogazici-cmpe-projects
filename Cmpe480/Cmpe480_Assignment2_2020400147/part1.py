...
def minimax_decision(node, selectedAgentNo):
    v = -1
    for expandedNode in successors(node):
        result = min_value(expandedNode, selectedAgentNo, 1)
        if(result > v):
            v = result
    node.eval = v
    return v
...
def max_value(node, selectedAgentNo, depth):
    ...
    v = -1
    for expandedNode in successors(node):
        v = max(v, min_value(expandedNode, selectedAgentNo, depth+1))
    node.eval = v
    return v

def min_value(node, selectedAgentNo, depth):
    ...
    v = 1
    for expandedNode in successors(node):
        v = min(v, max_value(expandedNode, selectedAgentNo, depth+1))
    node.eval = v
    return v
...
result = minimax_decision(rootNode, selectedAgentNo)
